...
- `--url` for `discover` is the agent base URL; the client appends
  ...
  as-is). For `send`/`wait` it is the JSON-RPC endpoint (the `url` from the
  Agent Card, e.g. `http://host:9900` or `http://host:18789/a2a/v1`).
...
| Code | Meaning |
| ---- | ------- |
| 0 | Success (`wait` reached `completed`). |
| 1 | Usage/configuration (bad flags, missing env, invalid URL). |
| 2 | Authentication (401/403 or missing bearer). |
| 3 | Protocol (version mismatch, bad content type, malformed envelope, missing fields). |
| 4 | Remote terminal task failure (`failed`, `rejected`, or `canceled`; payload still printed). |
